...
**Status:** Proposal complete as a dependency proposal. Pi implementation is blocked until an authoritative task-to-skill requirements contract is published by the pinned kernel and separately approved for consumption.
...
| Selection owner | The kernel owns `MissionIntent`-to-skill requirements and required/optional policy. Pi only consumes published descriptors and outcomes. |
...
| Memory | The Pi host/backend memory contract is a separate unresolved dependency. |
...
Make the eventual production path translate an actual `MissionIntent`, explicit `PE` product jurisdiction, and required fiscal `asOf` date into authoritative, version-pinned skills selected by the kernel. The result must be usable by Pi without Pi inventing a mission-to-skill mapping, optionality rule, checksum, fiscal date, or authorization decision.
...
- Define Pi's future acceptance boundary for successful selection, explicit no-skills-required, unsupported tasks, and resolution errors.
...
- Adding a Pi-local `MissionIntent`-to-skill mapping table, requirement policy, skill registry, checksum, or fallback.
...
2. Pi passes those values unchanged to a future public kernel task-requirements API.
...
4. Pi handles the kernel result without reinterpretation:
...
5. Optional-skill behavior is whatever the explicit kernel descriptor and resolver contract state. Pi must not infer optionality from an empty list or catch errors into a local default.

No outcome in this flow authorizes fiscal action. Pi remains an operator and consumer of kernel decisions, never an authority.
...
If `asOf` is missing or invalid, selection is blocked before any skill lookup. Pi must never default it to:
...
The current product jurisdiction is explicitly `PE`. RUC validation remains part of company scope discipline, but an 11-digit RUC is not an ISO-3166 jurisdiction source. Pi must not derive `PE` from the RUC or silently replace a missing jurisdiction.
...
The exact contract and Pi acceptance scenarios are listed in `upstream-contract-proposal.md`. The minimum production evidence is:
...
| Pi invents mappings or optionality | Fiscal policy diverges from the kernel and becomes unauditable. | Consume only explicit kernel descriptors and outcomes; no local table or fallback. |
...
- [x] Defines Pi's non-authorizing acceptance boundary and the separate memory dependency.
...
- [ ] A separately authorized dependency/pin release exposes that contract to Pi.
