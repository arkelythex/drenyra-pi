# Tasks: Pi Capability Conformance
...
- [x] Verify PR 1 with `bun test __tests__/capability-manifest.test.ts`, `bun run verify:capability`, `bun run typecheck`, and `bun run verify:style`; record each exact result, with runtime-harness verification explicitly `N/A` because this guard is offline and invokes no Pi runtime boundary. <!-- sdd-owner: implementation -->
