...
# Verification Report — Pi Capability Conformance
...
| REQ-CONF-002 (taxonomy disclosure) | — | PASS | Matrix states the taxonomy "is Drenyra Pi's own synthesis of Kubernetes-style evidence-gated readiness and Backstage-style manifest embedding. It is not a 1:1 copy of either standard." |
...
| REQ-CONF-007 | Ownership escalation is rejected | PASS | Guard rule `NON_LOCAL_OWNERSHIP` × `PI_LOCAL_OWNERSHIP`; plus `AUTHORITY_VALUES = {pi-operates-never-authorizes}` is enforced for every capability; plus a forbidden-phrase scan rejects any declared surface claiming Pi fiscal authority or Pi delivery of the master program. Independently reproduced: N2 (ownership escalation) and N5 (forbidden authority phrase). |
