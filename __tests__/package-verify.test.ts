...
function makeVendoredFixture(
	...
	writeFileSync(
		join(vendoredDir, options.tarballName ?? "drenyra-ai-0.4.0.tgz"),
		buildTarGz(tarFiles),
	);
...
describe("vendored runtime artifact reconciliation vs DEFAULT_PIN", () => {
	it("reconciles the REAL vendored drenyra-ai-0.4.0.tgz against the released DEFAULT_PIN", () => {
...
describe("tar reader", () => {
	...
	it("round-trips a synthetic tar.gz fixture", () => {
		...
		const fixture = makeVendoredFixture();
		const entry = readTarEntry(
			join(fixture.root, "vendored", "drenyra-ai-0.4.0.tgz"),
			"package/dist/cmd/cli.js",
		);
