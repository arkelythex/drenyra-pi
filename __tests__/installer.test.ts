...
import { decideInstall, runInstaller, type InstallerDeps } from "../runtime/installer.js";
...
describe("decideInstall", () => {
  it("pending-release: nothing to install, notice names the pin", () => {
    ...
    if (decision.kind === "pending-release") {
      ...
      expect(decision.notice).toContain("drenyra-ai@0.0.1-prealpha.1");
    }
  });
  ...
  it("released: install the exact pinned version, never a range", () => {
    ...
    if (decision.kind === "released") {
      ...
      expect(decision.version).toBe("0.0.1-prealpha.1");
    }
  });
});
...
describe("runInstaller", () => {
  it("pending-release: prints a clear notice and exits 0 without touching npm", async () => {
    const root = makeRoot();
    try {
      const result = await runInstaller({ pin: createPin(), packageRoot: root });
  ...
  it("released + doctor verified: exits 0", async () => {
    const root = makeRoot();
    try {
      const pin = createPin({ state: "released", checksumSha256: "b".repeat(64) });
  ...
  it("released + npm install failure: exits 1 with the npm error", async () => {
    const root = makeRoot();
    try {
      const pin = createPin({ state: "released", checksumSha256: "c".repeat(64) });
      const deps: InstallerDeps = {
        install: async () => {
  ...
  it("released + doctor not verified: exits 1 listing the doctor issues", async () => {
    const root = makeRoot();
    try {
      const pin = createPin({ state: "released", checksumSha256: "d".repeat(64) });
      const deps: InstallerDeps = {
        install: async () => undefined,
        verify: async () => ({
          pinState: "released",
