...
| --- | --- |
| Pinned exact version | `runtime/pin.ts` — `RuntimePin`, `createPin` (validates exact semver, hex checksum, state) and `DEFAULT_PIN` (package `drenyra-ai`, version `0.4.0`, state `released`; `checksumSha256` `09df8d696204337a9b62ddd28c354b414b62e81924caaf68a50b61131d5b7600` — SHA-256 of the release entry artifact `dist/cmd/cli.js`, per `runtime/doctor.ts`; v0.4.0 MINOR addition: configurator host integration + the routing preflight router `routing/router.ts`, which Pi's routing adapter consumes for the route decision) |
