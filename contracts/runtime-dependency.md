...
> Version: v0.2 · Status: frozen · Applies to: Drenyra Shell ↔ Drenyra AI.

This contract defines how Drenyra Shell consumes the Drenyra AI runtime. It follows the Gentle Shell ↔ Gentle AI pattern: **exact, verified, package-local** — never an ambient binary.

## Rules

1. **Pinned exact version.** The Drenyra AI version is fixed in the Drenyra Shell package manifest (or a lockfile). Range pins are not allowed for fiscal operations.

2. **Package-local install.** The runtime lives inside Drenyra Shell's own package tree:
...
6. **Upgrade is explicit.** Changing the pin is a release of Drenyra Shell itself, with:
   - a changelog entry,
   - a migration note describing contract impact,
   - re-run of `doctor` and conformance tests.

7. **Single source of truth.** The pinned runtime's contracts (`mission-protocol`, `candidate`, `receipt`, `gate` from `arkelythex/drenyra-ai/contracts`) are authoritative. Drenyra Shell never forks or redefines them.
